...
import cn.lemon.dubbo.message.util.EmailUtil;
			...
			try {
				switch (messageRecord.getPushMethod()) {
					case INL: // 发送站内消息
						...
						break;
					case SMS: // 发送短信
						//alismsUtil.send(messageRecord.getSendTo(), messageRecord.getCenterId(), messageRecord.getMessage());
						logger.info("短信发送成功。 {}", messageRecord.getMessage());
						break;
					case EMI: // 发送邮件消息
						emailUtil.send(messageRecord.getSendTo(), messageRecord.getTitle(), messageRecord.getMessage());
					...
					case RMQ: // 发送MQ消息
						...
						break;
					case WXM: // 发送微信模板消息
						weixinUtil.sendTemplateMessage(messageRecord.getMessage());
						break;
					case ANR: // 发送安卓消息
